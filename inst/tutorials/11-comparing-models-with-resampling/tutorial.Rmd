...
```{r creating-mulitple-mo-1-hint, eval = FALSE}
library(...)
```
...
```{r creating-mulitple-mo-2-hint, eval = FALSE}
...
```{r creating-mulitple-mo-3-hint, eval = FALSE}
... |>
  step_log(..., base = ...)
```
...
```{r creating-mulitple-mo-4-hint, eval = FALSE}
... |> 
  step_other(Neighborhood, threshold = 0.01)
```
...
```{r creating-mulitple-mo-5-hint, eval = FALSE}
basic_rec <- ... |>
  step_dummy(...())
```
...
```{r creating-mulitple-mo-6-hint, eval = FALSE}
interaction_rec <- 
  ... |>
...
```{r creating-mulitple-mo-7-hint, eval = FALSE}
spline_rec <- 
  ... |> 
...
```{r creating-mulitple-mo-8-hint, eval = FALSE}
... <- 
  list(basic = basic_rec,
       interact = ...,
       splines = spline_rec)
...
```{r creating-mulitple-mo-9-hint, eval = FALSE}
...
```{r creating-mulitple-mo-10-hint, eval = FALSE}
lm_models |>
  ...("...")
```
...
```{r creating-mulitple-mo-11-hint, eval = FALSE}
...
```{r creating-mulitple-mo-12-hint, eval = FALSE}
... <- 
  lm_models |>
  workflow_map("fit_resamples", 
               seed = ..., 
...
```{r creating-mulitple-mo-13-hint, eval = FALSE}
collect_metrics(...)
```
...
```{r creating-mulitple-mo-14-hint, eval = FALSE}
... |>
  filter(.metric == "...")
```
...
```{r creating-mulitple-mo-15-hint, eval = FALSE}
...
```{r creating-mulitple-mo-16-hint, eval = FALSE}
four_models <-
  ... |> 
  bind_rows(lm_models)
```
...
```{r creating-mulitple-mo-17-hint, eval = FALSE}
... |>
  autoplot(metric = "...")
```
...
```{r creating-mulitple-mo-18-hint, eval = FALSE}
... +
...
```{r creating-mulitple-mo-19-hint, eval = FALSE}
... +
...
```{r creating-mulitple-mo-20-hint, eval = FALSE}
... +
  theme_classic() +
  theme(... = "none")
```
