...
Resampling is a technique used in statistics and machine learning to assess the performance of models and estimate their generalization error. In R, common types of resampling include cross-validation (e.g., k-fold cross-validation), bootstrapping, and permutation testing.
...
The **caret** (Classification And REgression Training) package in R is a powerful tool for performing various resampling techniques. It provides a consistent interface for cross-validation, bootstrapping, and other resampling methods across different modeling techniques.
### Exercise 9
...
k-fold cross-validation is a widely used resampling technique in which the original dataset is divided into k subsets (folds). The model is trained k times, each time using k-1 folds for training and the remaining fold for validation.
...
```{r bayesian-methods-9-hint-1, eval = FALSE}
... |>
...
```{r bayesian-methods-10-hint-1, eval = FALSE}
...
```{r bayesian-methods-11-hint-1, eval = FALSE}
autoplot(...)
```
...
```{r bayesian-methods-12-hint-1, eval = FALSE}
...
```{r bayesian-methods-13-hint-1, eval = FALSE}
... +
...
Bootstrapping involves randomly sampling with replacement from the original data set to create multiple "bootstrap samples." Models are trained on these samples to estimate properties of the model, such as confidence intervals for model parameters. The **boot** package in R is often used for bootstrap re-sampling.
...
```{r bayesian-methods-14-hint-1, eval = FALSE}
... +
  theme(... = "...")
```
...
```{r bayesian-methods-15-hint-1, eval = FALSE}
rqs_diff <-
  contrast_models(...,
                  list_1 = "splines_lm",
                  list_2 = "basic_lm",
...
```{r bayesian-methods-16-hint-1, eval = FALSE}
rqs_diff |>
  ...() |>
  ...()
```
...
```{r bayesian-methods-17-hint-1, eval = FALSE}
rqs_diff  |>
  as_tibble() |> 
  ggplot(aes(x = ...))
```
...
When performing re-sampling, various metrics can be used to assess model performance, such as accuracy, mean squared error, area under the ROC curve (AUC), and more. These metrics help evaluate how well the model generalizes to new data.
...
```{r bayesian-methods-18-hint-1, eval = FALSE}
... +
...
R provides visualization tools to help understand the results of resampling. For instance, you can use the `plot` and `xyplot` functions to create visualizations of cross-validation results, showing how performance varies across different folds or iterations.
...
```{r bayesian-methods-19-hint-1, eval = FALSE}
... +
...
```{r bayesian-methods-20-hint-1, eval = FALSE}
summary(...)
```
...
```{r bayesian-methods-21-hint-1, eval = FALSE}
... |>
  ...(-starts_with("..."))
```
...
To estimate the probability of being practically significant, the size option will be used in the `summary()` function. Copy the previous code and add `size = 0.02` to `summary()` and add `contrast` to `select()`. Also, remove `-` from `starts_with()`.
...
```{r bayesian-methods-22-hint-1, eval = FALSE}
...
Type `autoplot()` and add the parameter `rsq_anova`. Hit "Run Code".

```{r bayesian-methods-23, exercise = TRUE}

```

```{r bayesian-methods-23-hint-1, eval = FALSE}
autoplot(...)
```
...
When `perf_mod()` is used with a workflow set, the `autoplot()` method can show the `pract_equiv` results that compare each workflow to the current best (the random forest model, in this case).

### Exercise 24

Copy the previous code and add the parameters `type`, setting it to `"ROPE"`, and `size`, setting it equal to `0.02`.

```{r bayesian-methods-24, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r bayesian-methods-24-hint-1, eval = FALSE}
autoplot(rsq_anova, type = "...", size = ...)
```
...
The `type` parameter allows you to choose from a variety of plot types that are suitable for the specific class of the input object. The available `type` options vary depending on the class of the object being plotted.

### Exercise 25

Copy the previous code and add `geom_text_repel()`. Within `aes()` of `geom_text_repel()`, add `label`, setting it equal to `workflow`.

```{r bayesian-methods-25, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r bayesian-methods-25-hint-1, eval = FALSE}
... +
  geom_text_repel(aes(... = workflow))
```
...
With a posterior distribution, we can also compute the probability of being practically significant. In Bayesian analysis, this is a *ROPE estimate* (for Region Of Practical Equivalence, Kruschke and Liddell (2018)). 

### Exercise 26

Copy the previous code and add `theme()`. Add the parameter `legend.position` and set it equal to `"none"`.

```{r bayesian-methods-26, exercise = TRUE}

```

<button onclick = "transfer_code(this)">Copy previous code</button>

```{r bayesian-methods-26-hint-1, eval = FALSE}
... +
  ...(legend.position = "...")
```
