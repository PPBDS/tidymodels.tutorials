...
This tutorial covers [Chapter 15: Grid Search](https://www.tmwr.org/workflow-sets) from [*Tidy Modeling with R*](https://www.tmwr.org/) by Max Kuhn and Julia Silge. This chapter will illustrate how to use workflow sets to investigate multiple models or feature engineering strategies in such a situation. Racing methods can more efficiently rank models than fitting every candidate model being considered.
...
```{r creating-the-workflo-1-hint, eval = FALSE}
workflow_set(
...
```{r, include = FALSE}
workflow_set(
  preproc = list(normalized = normalized_spec)
)
```
...
```{r creating-the-workflo-2-hint, eval = FALSE}
normalized <- 
  workflow_set(
    preproc = list(normalized = normalized_rec),
...
```{r creating-the-workflo-3-hint, eval = FALSE}
normalized |>
  extract_workflow(id = "...")
```
...
```{r creating-the-workflo-4-hint, eval = FALSE}
normalized1 <-
  normalized |>
...
```{r creating-the-workflo-5-hint, eval = FALSE}
model_vars <-
...
```{r creating-the-workflo-6-hint, eval = FALSE}
...
```{r creating-the-workflo-7-hint, eval = FALSE}
no_pre_proc <- 
  workflow_set(
    preproc = list(simple = model_vars), 
    models = list(MARS = ...,
...
```{r creating-the-workflo-8-hint, eval = FALSE}
workflow_set(
  preproc = ...(full_quad = ...)
)
```
...
```{r creating-the-workflo-9-hint, eval = FALSE}
with_features <- 
  workflow_set(
    preproc = list(full_quad = poly_recipe), 
...
```{r creating-the-workflo-10-hint, eval = FALSE}
...
```{r creating-the-workflo-11-hint, eval = FALSE}
all_workflows <-
  ... |>
...
```{r tuning-and-evaluatin-1-hint, eval = FALSE}
grid_ctrl <-
   control_grid(
      save_pred = ...,
      parallel_over = "...",
...
```{r tuning-and-evaluatin-2-hint, eval = FALSE}
all_workflows |>
  ...()
```
...
```{r tuning-and-evaluatin-3-hint, eval = FALSE}
grid_results <-
   ... |>
   workflow_map(
      seed = ...,
...
```{r tuning-and-evaluatin-4-hint, eval = FALSE}
grid_results |>
  ...() |>
  filter(.metric = "...")
```
...
```{r tuning-and-evaluatin-5-hint, eval = FALSE}
... |>
...
```{r tuning-and-evaluatin-6-hint, eval = FALSE}
autoplot(
  grid_results,
  rank_metric = "...",
  metric = "rmse",
...
```{r tuning-and-evaluatin-7-hint, eval = FALSE}
... +
...
```{r tuning-and-evaluatin-8-hint, eval = FALSE}
... +
...
```{r tuning-and-evaluatin-9-hint, eval = FALSE}
... +
  lims(y = c(..., ...))
```
...
```{r tuning-and-evaluatin-10-hint, eval = FALSE}
... +
...
```{r tuning-and-evaluatin-11-hint, eval = FALSE}
...
```{r efficiently-screenin-1-hint, eval = FALSE}
race_ctrl <-
  control_race(
    save_pred = ...,
    parallel_over = "...",
...
```{r efficiently-screenin-2-hint, eval = FALSE}
all_workflows |>
  workflow_map(
    "..."
  )
...
```{r efficiently-screenin-3-hint, eval = FALSE}
race_results <-
  ... |>
  workflow_map(
    "tune_race_anova",
...
```{r efficiently-screenin-4-hint, eval = FALSE}
autoplot(
  race_results,
  rank_metric = "...",  
  metric = "...",       
...
```{r efficiently-screenin-5-hint, eval = FALSE}
... +
  geom_text(aes(y = ..., label = ...))
```
...
```{r efficiently-screenin-6-hint, eval = FALSE}
... +
  lims(y = c(..., ...))
```
...
```{r efficiently-screenin-7-hint, eval = FALSE}
... +
  theme(... = "none")
```
...
```{r efficiently-screenin-8-hint, eval = FALSE}
...
```{r efficiently-screenin-9-hint, eval = FALSE}
... |>
  select(wflow_id, .metric, race = mean, config_race = .config)
```
...
```{r efficiently-screenin-10-hint, eval = FALSE}
... |>
  inner_join(
    rank_results(..., select_best = ...)
  )
...
```{r efficiently-screenin-11-hint, eval = FALSE}
... |>
  inner_join(
    ... |>
...
```{r efficiently-screenin-12-hint, eval = FALSE}
... |>
  inner_join(
    ...,
    by = c("...", "...")
...
```{r efficiently-screenin-13-hint, eval = FALSE}
matched_results <-
  ... |>
  filter(.metric == "...")
```
...
```{r efficiently-screenin-14-hint, eval = FALSE}
matched_results |>
  ggplot(aes(x = ..., y = ...))
```
...
```{r efficiently-screenin-15-hint, eval = FALSE}
... |>
  geom_abline(lty = ...)
```
...
```{r efficiently-screenin-16-hint, eval = FALSE}
... +
  ...() +
...
```{r efficiently-screenin-17-hint, eval = FALSE}
... +
  coord_obs_pred() +
  labs(x = "...", y = "...")
```
...
```{r finalizing-a-model-1-hint, eval = FALSE}
race_results |>
...
```{r finalizing-a-model-2-hint, eval = FALSE}
best_results <-
  ... |>
  select_best(metric = "rmse")
```
...
```{r finalizing-a-model-3-hint, eval = FALSE}
race_results |>
  extract_workflow("...") |>
  finalize_workflow(...)
```
...
```{r finalizing-a-model-4-hint, eval = FALSE}
boosting_test_results <-
  ... |>
  last_fit(split = ...)
```
...
```{r finalizing-a-model-5-hint, eval = FALSE}
collect_metrics(...)
```
...
```{r finalizing-a-model-6-hint, eval = FALSE}
boosting_test_results |>
  ...() |>
  ggplot(aes(x = ..., y = .pred))
```
...
```{r finalizing-a-model-7-hint, eval = FALSE}
... +
...
```{r finalizing-a-model-8-hint, eval = FALSE}
... +
  geom_point(alpha = ...) +
  coord_orbs_pred() +
  labs(x = "...", y = "...")
