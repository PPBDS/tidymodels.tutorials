...
Copy the previous code and change to `formula` argument to `rate ~ temp + time`.
...
```{r modeling-in-base-r-10-hint, eval = FALSE}
lm(formula = rate ~ temp, data = crickets)

```

```{r include=FALSE}
lm(formula = rate ~ temp + time, data = crickets)
```

### 

The `rate ~ temp + time` formula would not add the time and temperature values together. Instead, this formula would symbolically represent that temperature and time should be added as separate *main effects* to the model. A *main effect* is a model term that contains a single predictor variable.

### Exercise 11

The `species` column can be added to the fitted model using the same syntax as we added `time`. Copy the previous code and change `time` to `species`.
