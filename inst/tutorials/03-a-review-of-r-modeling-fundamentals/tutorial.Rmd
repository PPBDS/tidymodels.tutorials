...
In the code, pipe `corr_res` to the `map_dfr()` funciton. Inside `map_dfr(), type in `tidy` and set the `.id` argument to `"predictor"`.
...
```{r model-implementation-20-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
    ggplot(...())
```
...
```{r model-implementation-21-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
...
```{r model-implementation-22-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
    ggplot(aes(x = fct_reorder(predictor, estimate)))  + 
      geom_point(...(y = ...))
...
```{r model-implementation-23-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
    ggplot(aes(x = fct_reorder(predictor, estimate))) + 
      geom_point(aes(y = estimate)) + 
...
```{r model-implementation-24-hint, eval = FALSE}
corr_res |>
    map_dfr(tidy, .id = "predictor") |>
    ggplot(aes(x = fct_reorder(predictor, estimate))) + 
    geom_point(aes(y = estimate)) + 
...
```{r model-implementation-25-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
    ggplot(aes(x = fct_reorder(predictor, estimate))) + 
      geom_point(aes(y = estimate)) + 
...
```{r model-implementation-26-hint, eval = FALSE}
corr_res |>
  map_dfr(tidy, .id = "predictor") |>
    ggplot(aes(x = fct_reorder(predictor, estimate))) + 
      geom_point(aes(y = estimate)) + 
