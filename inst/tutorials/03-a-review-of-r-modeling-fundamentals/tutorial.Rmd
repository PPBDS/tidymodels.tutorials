...
We can use this function to fit separate models for each of the species in the `cricket` tibble.
...
```{r combining-base-r-mod-2-hint, eval = FALSE}
... |>
  group_nest(...)
```
...
```{r combining-base-r-mod-3-hint, eval = FALSE}
... <- crickets |>
  group_nest(species)
```
...
```{r combining-base-r-mod-4-hint, eval = FALSE}
