...
Lets add this to `step_ames1`, which was created earlier in this section. Type in `step_ames1` and press "Run Code". 

```{r examples-of-recipe-s-16, exercise = TRUE}

```

```{r examples-of-recipe-s-16-hint, eval = FALSE}
...
```
...
```{r examples-of-recipe-s-17-hint, eval = FALSE}
simple_ames1 |>
...
```{r examples-of-recipe-s-18-hint, eval = FALSE}
... <- simple_ames1 |>
  step_interact( ~ Gr_Liv_Area:starts_with("Bldg_Type_"))
```
...
```{r examples-of-recipe-s-20-hint, eval = FALSE}
library(...)
```
...
```{r examples-of-recipe-s-21-hint, eval = FALSE}
library(...)
```
...
```{r examples-of-recipe-s-22-hint, eval = FALSE}
function (deg_free) {
  ...()
}
```
...
```{r examples-of-recipe-s-23-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ...)
}
```
...
```{r examples-of-recipe-s-24-hint, eval = FALSE}
function (deg_free) {
...
```{r examples-of-recipe-s-25-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2)
}
...
```{r examples-of-recipe-s-26-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2) + 
    ...()
...
```{r examples-of-recipe-s-27-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2) + 
    scale_y_log10() + 
...
```{r examples-of-recipe-s-28-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2) + 
    scale_y_log10() + 
...
```{r examples-of-recipe-s-29-hint, eval = FALSE}
function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2) + 
    scale_y_log10() + 
...
```{r examples-of-recipe-s-30-hint, eval = FALSE}
... <- function (deg_free) {
  ggplot(data = ames_train, aes(x = Latitude, y = 10^Sale_Price)) + 
    geom_point(alpha = 0.2) + 
    scale_y_log10() + 
...
As you can, this code displays a variety of Spline Term graphs. However,the "2 Spline Terms" graph *underfit* the data while the "100 Spline Terms" graph *overfit* the data. This indicates that the proper amount of “nonlinear-ness” matters.
