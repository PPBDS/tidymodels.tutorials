...
```{r the-resubstitution-a-8-hint, eval = FALSE}
... <- rand_forest(trees = 1000) |>
  set_engine("ranger") |>
  set_mode("regression")
```
...
```{r the-resubstitution-a-9-hint, eval = FALSE}
workflow() |>
  ...()
```
...
```{r the-resubstitution-a-10-hint, eval = FALSE}
workflow() |>
...
```{r the-resubstitution-a-11-hint, eval = FALSE}
workflow() |>
  add_formula(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type + Latitude + Longitude) |>
  add_model(...)
```
...
```{r the-resubstitution-a-12-hint, eval = FALSE}
... <- workflow() |>
  add_formula(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type + Latitude + Longitude) |>
  add_model(rf_model)
```
...
```{r the-resubstitution-a-13-hint, eval = FALSE}
rf_wflow |>
  fit(... = ...)
```
...
```{r the-resubstitution-a-14-hint, eval = FALSE}
... <- rf_wflow |>
  fit(data = ames_train)
```
