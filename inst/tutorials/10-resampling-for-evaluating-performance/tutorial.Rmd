...
In the code chunk below, type in `vfold_cv()`, passing in `ames_train` and setting `v` to `10`.
...
Lets create a validation set object that uses 3/4 of the data for model fitting. In the code chunk below, type in `validation_set()`, passing in `ames_train` and setting `prop` to `3/4`.
...
.__C__.environment <- validation_split(ames_train, prop = 3/4)
```
...
Lets create a plot. Start by piping `assess_res` to `ggplot()`. 
