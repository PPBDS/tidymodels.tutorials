...
As a reminder, here is the Heatmap of the mean area under the ROC curve for a high density grid of tuning parameter values:
...
The functions that are used to interrogate the results are the same as those used for grid search. Type `?show_best()` in the Console and look at the *Descripton* section. CP/CR.
...
```{r simulated-annealing-1-hint, eval = FALSE}
library(...)
```
...
Type `?control_sim_anneal()` in the Console and look at the *Descripton* section. CP/CR.
...
Next, type `?tune_sim_anneal()` in the Console and look at the *Descripton* section. CP/CR.
...
```{r simulated-annealing-4-hint, eval = FALSE}
...
How are acceptance probabilities influenced? The heatmap in the graph below shows how the acceptance probability can change over iterations, performance, and the user-specified coefficient:
...
```{r simulated-annealing-5-hint, eval = FALSE}
...
```{r simulated-annealing-6-hint, eval = FALSE}
set.seed(...)
```
...
```{r simulated-annealing-7-hint, eval = FALSE}
... |>
  tune_sim_anneal(
    resamples = ...
  )
...
```{r simulated-annealing-8-hint, eval = FALSE}
svm_wflow |>
  tune_sim_anneal(
    resamples = cell_folds,
    metrics = ...
