...
This tutorial covers [Chapter 16: Dimensionality Reduction](https://www.tmwr.org/dimensionality) from [*Tidy Modeling with R*](https://www.tmwr.org/) by Max Kuhn and Julia Silge. In this tutorial, you will learn about creating correlation matrices, with the use of `corrplot()`, prepping and baking recipes, with the use of `prep()` and `bake()`, feature extraction tecniques such as PCA, PLS, ICA, and UMAP, and advanced modeling, using functions such as `finalize_workflow()` and `workflow_map()`. The data that is used in this tutorial comes from the [**beans**](https://cran.r-project.org/web/packages/beans/index.html) package. There are many packages used in this tutorial, some of which are [**corrplot**](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html), [**mixOmics**](https://www.bioconductor.org/packages/release/bioc/html/mixOmics.html), and [**embed**](https://cran.r-project.org/web/packages/embed/index.html).
...
```{r a-picture-is-worth-a-3-hint, eval = FALSE}
library(...)
library(...)
library(...)
```
...
In general, if you encounter errors when working with a recipe, `prep()` can be used with its `verbose` option to troubleshoot.
...
Now, lets process `bean_validation` with `bake()`. In the code chunk below, type in `bake()`. Inside this function, type in `bean_rec_trained` and set `new_data` to `bean_validation`.
...
`ggtitle()` is another way to add a title to a graph. `labs()` also has the capability of doing this, but it's good to know other ways as well. 
...
Another type of dimensionality reduction technique is t-Distributed Stochastic Neighbor Embedding, or t-SNE. t-SNE is particularly useful for reducing high-dimensional data to two or three dimensions, which can then be visualized.
...
As a reminder, `scale_color_brewer()` is a function that provides sequential, diverging and qualitative color schemes.
...
```{r modeling-31-hint-1, eval = FALSE}
rank_results(bean_res, select_best = TRUE) |>
...
```{r modeling-32-hint-1, eval = FALSE}
... <-
  rank_results(bean_res, select_best = TRUE) |>
  mutate(method = map_chr(wflow_id, ~ str_split(.x, "_", simplify = TRUE)[1]))
```
...
```{r modeling-33-hint-1, eval = FALSE}
filter(rankings, ... <= ...)
```
...
```{r modeling-34-hint-1, eval = FALSE}
filter(rankings, rank <= 5) |>
...
```{r modeling-36-hint-1, eval = FALSE}
bean_res |> 
  extract_workflow("...")
```
...
```{r modeling-37-hint-1, eval = FALSE}
bean_res |> 
  extract_workflow("pls_rda") |>
  finalize_workflow(
    ... |> 
...
```{r modeling-38-hint-1, eval = FALSE}
bean_res |> 
  extract_workflow("pls_rda") |>
  finalize_workflow(
    bean_res |> 
...
```{r modeling-39-hint-1, eval = FALSE}
bean_res |> 
  extract_workflow("pls_rda") |>
  finalize_workflow(
    bean_res |> 
...
```{r modeling-40-hint-1, eval = FALSE}
... <- 
  bean_res |> 
  extract_workflow("pls_rda") |>
  finalize_workflow(
...
```{r modeling-41-hint-1, eval = FALSE}
collect_metrics(...)
```
...
- How to create a correlation matrix using `corrplot()`
- How to prep and bake a recipe using `prep()` and `bake()`
- Various feature extraction techniques, such as PCA, PLS, ICA, and UMAP
- How to perform advanced modeling, using functions such as `workflow_map()` and `finalize_workflow()`
