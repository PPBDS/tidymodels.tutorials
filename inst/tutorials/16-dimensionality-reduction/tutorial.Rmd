...
```{r recipes-in-the-wild-1-hint, eval = FALSE}
prep(...)
```
...
```{r recipes-in-the-wild-2-hint, eval = FALSE}
... <- prep(bean_rec)
```
...
```{r recipes-in-the-wild-4-hint, eval = FALSE}
bean_rec |> 
  prep(... = ...)
```
...
```{r recipes-in-the-wild-5-hint, eval = FALSE}
bean_val$splits |>
  ...(1)
```
...
```{r recipes-in-the-wild-6-hint, eval = FALSE}
bean_val$splits |>
  pluck(1) |>
  ...()
```
...
```{r recipes-in-the-wild-7-hint, eval = FALSE}
... <- 
  bean_val$splits |>
  pluck(1) |>
  assessment()
...
```{r recipes-in-the-wild-8-hint, eval = FALSE}
bake(..., new_data = ...)
```
...
```{r recipes-in-the-wild-9-hint, eval = FALSE}
... <- bake(bean_rec_trained, new_data = bean_validation)
```
...
```{r recipes-in-the-wild-10-hint, eval = FALSE}
bean_validation |> 
  ...(aes(x = ...))
```
...
```{r recipes-in-the-wild-11-hint, eval = FALSE}
bean_validation |> 
  ggplot(aes(x = area)) +
...
```{r recipes-in-the-wild-12-hint, eval = FALSE}
bean_validation |> 
  ggplot(aes(x = area)) +
...
```{r recipes-in-the-wild-13-hint, eval = FALSE}
bean_validation |> 
  ggplot(aes(x = area)) +
  geom_histogram(bins = 30, alpha = 1/3, color = "white", fill = "blue") +
  ggtitle("...")
...
```{r recipes-in-the-wild-14-hint, eval = FALSE}
bean_val_processed |>
  ...(aes(x = ...))
```
...
```{r recipes-in-the-wild-15-hint, eval = FALSE}
bean_val_processed |>
  ggplot(aes(x = area)) +
...
```{r recipes-in-the-wild-16-hint, eval = FALSE}
bean_val_processed |>
  ggplot(aes(x = area)) +
  geom_histogram(bins = 30, alpha = 1/3, color = "white", fill = "red") +
  ggtitle("...")
