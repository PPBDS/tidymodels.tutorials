...
```{r common-method-for-sp-10-hint, eval = FALSE}
...(training(ames_split))
```
...
```{r stratified-sampling-6-hint, eval = FALSE}
ames_update |> 
  ggplot(aes(x = Sale_Price)) + 
    ...()
```
...
```{r stratified-sampling-7-hint, eval = FALSE}
ames_update |> 
  ggplot(aes(x = Sale_Price)) + 
    geom_density() + 
      ...()
...
```{r stratified-sampling-8-hint, eval = FALSE}
ames_update |> 
  ggplot(aes(x = Sale_Price)) + 
    geom_density() + 
      theme_classic() + 
...
```{r stratified-sampling-9-hint, eval = FALSE}
set.seed(...)
```
...
```{r stratified-sampling-10-hint, eval = FALSE}
...
```{r stratified-sampling-11-hint, eval = FALSE}
... <- initial_split(ames, prop = 0.80, strata = Sale_Price)
```
...
```{r stratified-sampling-12-hint, eval = FALSE}
...
```{r stratified-sampling-13-hint, eval = FALSE}
dim(...(ames_strata_split))
```
...
[Chapter 5 of the *Tidy Modeling With R*](https://www.tmwr.org/splitting.html) contains more information regarding how to spend data. Click on the link and type in the names of sections 5.2 - 5.4.
