...
ames_test_res <- ames_test_res |> rename(Sale_Price = Sale_Price)
...
<!-- PK: Having trouble with the column `Sale_Price` in data set `ames_test_res` -->
...
<!-- PK: Don't know why its throwing an error here. -->
```{r include = FALSE}
# ames_test_res |>
...
We will be using `rmse()` to compute the Root-Mean Square Error of `ames_test_res`. Pipe `rsme()` to `ames_test_res`. This might throw an error.
...
```{r multiclass-classific-7-hint, eval = FALSE}
roc_auc(hpc_cv, ...)
```
...
```{r multiclass-classific-8-hint, eval = FALSE}
...
```{r multiclass-classific-9-hint, eval = FALSE}
hpc_cv |>
  group_by(...)
```
...
```{r multiclass-classific-10-hint, eval = FALSE}
... |>
  accuracy(obs, ...)
```
...
```{r multiclass-classific-11-hint, eval = FALSE}
... |>
...
```{r multiclass-classific-12-hint, eval = FALSE}
hpc_cv |>
  group_by(Resample) |>
  roc_curve(obs, VF, F, M, L) |>
  ...()
