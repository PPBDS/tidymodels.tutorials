...
<!-- DK: Use test chunks for all questions. May not always be possible. PK: Done.-->
...
The *strucutre* of a function is as follows: 
...
```{r design-for-the-pipe--8-hint, eval = FALSE}
...
```{r design-for-the-pipe--9-hint, eval = FALSE}
...
```{r design-for-the-pipe--10-hint, eval = FALSE}
...{
  ...()
}
```
...
```{r design-for-the-pipe--11-hint, eval = FALSE}
...{
  return(...(mpg/wt))
}
```
...
Copy the previous code and add the base as a parameter for `log()`, which is equal to the log_base argument set in the function parameters for `compute_log_ratio`.
...
```{r design-for-the-pipe--12-hint, eval = FALSE}
...{
...
We must call the function outside of the function. Set `result` to `compute_log_ratio()`.
...
```{r design-for-the-pipe--13-hint, eval = FALSE}
result <- ...()
```
...
```{r design-for-the-pipe--14-hint, eval = FALSE}
result <- compute_log_ratio(mtcars$..., mtcars$...)
```
<!-- PK: Throwing an error here -->
```{r include = FALSE}
#result <- compute_log_ratio(mtcars$mpg, mtcars$wt)
```
...
Finally, print this value. You should get a series of values with the compute log ratios between `mpg` and `wt`.
...
```{r design-for-the-pipe--15-hint, eval = FALSE}
print(...)
```
...
In R, the head() function views first few rows of a data frame or a vector without displaying the entire dataset. 
...
```{r design-for-the-pipe--16-hint, eval = FALSE}
head(mtcars$mpg, ...)
```
...
```{r design-for-the-pipe--17-hint, eval = FALSE}
...|>
  map(sqrt)
```
...
map_dbl() returns the a vector compared to a list returned when map() is called. Copy the previous code and change map() to map_dlb().
...
```{r design-for-the-pipe--18-hint, eval = FALSE}
head(mtcars$mpg, 3)|>
  map_dbl(sqrt)
```
...
map2_dbl() operates across multiple vectors. Within the function, there are three parameters, in this case: two vectors and the function that must be used on those two vectors. Within map2_dbl(), enter the `mpg` and `wt` columns from mtcars, with the third function parameter being `compute_log_ratio()`.

```{r design-for-the-pipe--19, exercise = TRUE}
mtcars
```


```{r design-for-the-pipe--19-hint, eval = FALSE}
...
Copy the previous code. Select the columns `stationname, date, rides` using select(). 
