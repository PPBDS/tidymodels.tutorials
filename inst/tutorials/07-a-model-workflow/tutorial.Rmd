...
```{r how-does-a-workflow--2-hint, eval = FALSE}
...
```{r how-does-a-workflow--3-hint, eval = FALSE}
library(multilevelmod)
```
###
...
```{r how-does-a-workflow--4-hint, eval = FALSE}
multilevel_spec <- 
  linear_reg() |>
  set_model("...")
```
...
```{r how-does-a-workflow--5-hint, eval = FALSE}
workflow()
```
...
```{r how-does-a-workflow--6-hint, eval = FALSE}
... |>
  add_variables()
```
...
```{r how-does-a-workflow--7-hint, eval = FALSE}
... |>
  add_variables(outcome = ...)
```
...
```{r how-does-a-workflow--8-hint, eval = FALSE}
... |>
  add_variables(
    outcome = distance,
...
```{r how-does-a-workflow--9-hint, eval = FALSE}
... |>
  add_model()
```
...
```{r how-does-a-workflow--10-hint, eval = FALSE}
... |>
  add_model(multilevel_spec)
```
...
```{r how-does-a-workflow--11-hint, eval = FALSE}
... |>
  add_model(
    multilevel_spec,
    formula = ...
...
```{r how-does-a-workflow--12-hint, eval = FALSE}
multilevel_workflow <- ...
```
...
```{r how-does-a-workflow--13-hint, eval = FALSE}
fit(...)
```
...
```{r how-does-a-workflow--14-hint, eval = FALSE}
...
<!-- PK: Not sure if I should just give this code since it is kind of repetitive from the last 13 exercises of just split it up. -->
...
```{r creating-multiple-wo-2-hint, eval = FALSE}
library(...)
```
...
```{r creating-multiple-wo-3-hint, eval = FALSE}
...
We will use a linear model `lm_model` across the list of locations. Copy the previous code and add the parameter `models`, setting it equal to `list(lm = lm_model)`.
...
```{r creating-multiple-wo-4-hint, eval = FALSE}
